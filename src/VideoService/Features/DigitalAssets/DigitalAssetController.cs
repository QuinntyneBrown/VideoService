using VideoService.Features.DigitalAssets.UploadHandlers;
...
using static VideoService.Features.DigitalAssets.GetDigitalAssetByUniqueIdQuery;
using static VideoService.Features.DigitalAssets.AzureBlobStorageDigitalAssetCommand;
...
namespace VideoService.Features.DigitalAssets
{
    ...
    {        
        ...
        public async Task<IHttpActionResult> Upload(HttpRequestMessage request)
        {
            ...
            return Ok(await _mediator.Send(new AzureBlobStorageDigitalAssetRequest() { Provider = provider, TenantUniqueId = Request.GetTenantUniqueId() }));
        }
